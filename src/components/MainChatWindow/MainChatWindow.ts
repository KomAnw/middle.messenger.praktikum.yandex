...
import { ICustomEvent, Message as MessageType, SelectedChat } from 'src/modules/Store/types';
import { WSS } from 'src/modules/Fetch/ws';
import styles from './styles.module.scss';
import template from './MainChatWindow.html';
...
export class MainChatWindowComponent<P extends Props> extends Component<P> {
  ...
  async pushMessage({ user_id, time, content }: MessageType) {
    ...
    instance && this.getNode.querySelector('.chatMessages')?.prepend(instance.getNode);
  }
  ...
  onChatsLoaded(socket: WebSocket) {
    socket.addEventListener('message', (event) => {
      ...
      Array.isArray(response) ? this.renderMessages(response) : this.pushMessage(response);
    });
  }

  subscribe() {
    const button = this.getNode.querySelector('.sendButton')! as HTMLButtonElement;
